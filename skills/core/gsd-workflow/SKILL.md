...
### Subagent Spawning
Ensure you supply clear file lists when spawning subagents, as flash models benefit from explicit scoping. Stop immediately if looping is detected during execution.

[model:gemini-1.5-pro]
### Plan Verification
Pro models are excellent at finding edge cases in plans. Always review the spec for architectural inconsistencies during Phase 2 (Plan).
...
### Context Cleansing
You can periodically restart Claude Code or start a new prompt session while keeping `task.md` updated as the ground truth.
