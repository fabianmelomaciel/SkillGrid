...
- 2026-06-26 - Full Install + Bugfix + Vercel Agent Browser Analysis — (1) Cambiado default de installer PS1 de `minimal` a `all` (install.sh ya tenía `all`). (2) Bugfix: test `project-mixed-findings` fallaba por expected snapshot desincronizado con fixture (esperaba applied=3|pending=1, la fixture sin .fixture-answers.json daba applied=2|pending=2) — corregido snapshot. Todos los 226 tests OK. (3) Vercel Agent Browser (`vercel-labs/agent-browser`, 30K+ stars, v0.30.1) evaluado: arquitectura Rust CLI+Node.js daemon, snapshots por accessibility tree con refs, ideal para integración con `playwright-testing`, `execution-runtime`, o nueva skill de browser-automation. Token savings baseline: ~90% con CodeGraph en todas las plataformas. Mejor relación inteligencia/costo: antigravity + Gemini 2.5 Flash ($0.54/session con CG). Gotchas: no hay `.fixture-answers.json` en ningún fixture de audit-loop; todas las respuestas amarillas quedan pendientes por defecto.
