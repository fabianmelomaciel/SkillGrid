...
- 2026-07-13 - v1.8.0 GitHub Pentest Workflow (agente-ideas) — Deliberación 3 etapas evaluó 3 enfoques (Simplicidad/GitHub native, Seguridad/workflow dedicado, FinOps/diff scan). Ganó: Propuesta B (8/10) + optimizaciones de C. Entregado: `.github/workflows/pentest.yml` con 5 jobs: secrets-scan (TruffleHog+Gitleaks), sast-scan (Semgrep p/owasp-top-ten, SARIF), sca-scan (Trivy, bloquea CRITICAL), iac-scan (Checkov sobre workflows), supply-chain (OpenSSF Scorecard, solo push/schedule). README actualizado con sección `🔒 Security Pipeline` + badges CI/pentest. CHANGELOG v1.8.0. package.json bump 1.7.3→1.8.0. Tag v1.8.0 pusheado. Pre-commit gate: 227+9 tests OK. Instalación local completa (45 skills) en opencode, antigravity-ide, Claude Code. Gotcha: ossf/scorecard-action requiere `persist-credentials: false` en checkout + `SCORECARD_TOKEN: ${{ secrets.GITHUB_TOKEN }}`; correr solo en push/schedule para ahorrar minutos CI (no en cada PR).
- 2026-07-14 - v1.9.0 Loop-Engineering Skills (agente-ideas) — Deliberación 3 etapas evaluó 7 patrones de cobusgreyling/loop-engineering para integrar como skills. Ranking: 1º Changelog Drafter (9.5/10), 2º Issue Triage (8.0/10), 3º Post-Merge Cleanup (7.0/10 con gates L1). Vetados: Dependency Sweeper (supply chain), PR Babysitter (auto-merge), CI Sweeper (alto costo + riesgo SARIF). Creadas 3 nuevas skills: `changelog-drafter` (post-tag auto-changelog, read-only tools, secret scan gate, PR output), `issue-triage` (read-only issue classifier, gh CLI, L1 report-only default), `post-merge-cleanup` (stale branch scanner, branch whitelist, merge verification, worktree exclusion, human gate). Catálogo: 45→48 skills (core 28→31). Versión bump 1.8.0→1.9.0.
