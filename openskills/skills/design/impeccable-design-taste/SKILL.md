...
> **Full audit checklist:** See `references/design-audit-checklist.md` (Typography, Color, Spacing, Polish, Motion, Accessibility)
...
> **Implementaciones completas** (RevealStagger, StickyStack, CSS tokens) → `references/motion-patterns.md`

**Reglas clave (siempre en contexto):**
- Animaciones ≤300ms, solo `transform`/`opacity`, `cubic-bezier` custom
- `prefers-reduced-motion` obligatorio en todo lo que supere `MOTION_INTENSITY > 3`
- Nunca `window.addEventListener("scroll", ...)` — usar Motion `useScroll()`, GSAP ScrollTrigger, o CSS scroll-driven
- Stagger en lista: ok. Stagger idéntico en cada sección: AI tell
- Nunca animar imágenes en hover — animar la card (bg, border, shadow)
